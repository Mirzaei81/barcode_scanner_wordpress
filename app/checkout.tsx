...
export default function App(){
    const [checkout,setCheckout] = useState({name:"",familyName:"",phoneNumber:"",province:"",city:"",postAddress:"",paymentMethod:paymentMethods.parsian,transfer:transferMethods.post})
    const [cards,setCards] = useState<card[]>([])
    const [assets, error] = useAssets([require("../assets/images/Title.png"),])
    const [provinces,setProvinces] = useState<Option[]>([])
    const [states,setStates] = useState<Option[]>([])
    const [subscribe,setSubscribe] = useState(false)
    const [eulo,setEulo] = useState(false)
    const {cost} = useSearchParams()
    
    useEffect(()=>{
        (async()=>{
            const name =await getItemAsync("name")??''
            const familyName = await getItemAsync("familyName")??""
            ...
            setCheckout({ name, familyName, phoneNumber, province, city, postAddress, paymentMethod, transfer })
            const db = await Sqlite.openDatabaseAsync('Products.db');
            const storeCards:card[]=  await db.getAllAsync("SELECT * from cards;")
            storeCards.push({ name: "test", created_at: Date.now().toLocaleString(), id: 0, owner: "test", number: "1234566789" }) // remember me 
            setCards([...storeCards])
    ...
    const setProvince = async (p: string) => {
        ...
        <SafeAreaView>
            <View style={{height:"100%",flexDirection:"column",backgroundColor:colorMain}}>
                ...
                <View style={styles.main}>
                    <ScrollView>
                        <View style={{ display: "flex", direction: "rtl", alignSelf:"flex-end"}}>
                            <Text style={{ padding: 5, width: "100%", borderWidth: 1, fontSize: 22, color: "#008000ff", borderColor: "#008000ff", borderRadius: 10 }}>آدرس جدید</Text>
                        </View>
                        <View style={{ marginTop:20,direction: "rtl", display: "flex", flexDirection: "row",justifyContent: "space-between" }}>
                            <View style={{ width: "40%",marginLeft:10 }}>
                                ...
                                <TextInput onChangeText={(e)=>setCheckout({...checkout,name:e})} theme={{roundness:10}} style={{backgroundColor:"white",height:40}} mode="outlined" />
                            </View>
                            <View style={{ width: "55%" }}>
                                <Text style={{fontSize:18}}>نام خانوادگی</Text>
                                <TextInput onChangeText={(e)=>setCheckout({...checkout,familyName:e})}theme={{roundness:10}} style={{backgroundColor:"white",height:40}} mode="outlined" />
                            </View>
                        </View>
                        <View style={{ direction: "rtl", display: "flex", flexDirection: "row"}}>
                            <View style={{ width: "100%", marginLeft: 10 }}>
                                <Text style={{ fontSize: 18 }}>شماره تماس</Text>
                                <TextInput onChangeText={(e)=>setCheckout({...checkout,phoneNumber:e})} theme={{ roundness: 10 }} style={{ backgroundColor: "white",height:40 }} mode="outlined" />
                            </View>
                        </View>
                        <View style={{display:"flex",flexDirection:"row",marginTop:25,justifyContent:"center"}}>
                            <View style={{ width: "45%",marginRight:10 }}>
                                <Dropdown hideMenuHeader menuContentStyle={{backgroundColor:colorSecondary}} value={checkout.city}  disabled={states.length == 0} mode="outlined" placeholder="شهر" options={states} onSelect={(d) =>setCheckout({...checkout,city:d??""}) } />
                            </View>
                            <View style={{ width: "45%" }}>
                                <Dropdown hideMenuHeader menuContentStyle={{backgroundColor:colorSecondary}} value={checkout.province} disabled={provinces.length==0} mode="outlined" placeholder="استان" options={provinces} onSelect={(d) => setProvince(d ?? "")} />
                            </View>
                        </View>
                        <View style={{ direction: "rtl", display: "flex", flexDirection: "row",marginBottom:4}}>
                            <View style={{ width: "100%", marginLeft: 10 }}>
                                <Text style={{ fontSize: 18 }}>ادرس پستی</Text>
                                <TextInput contentStyle={{direction:"rtl"}} onChangeText={(e)=>setCheckout({...checkout,postAddress:e})} theme={{ roundness: 10 }} style={{ backgroundColor: "white",height:40,direction:"rtl" }} mode="outlined" />
                            </View>
                        </View>
                        <View style={{display:"flex",flexDirection:"row",justifyContent:"space-around"}}>
                            ...
                            <View>
                                ...
                                {Object.keys(transferMethods).map((p)=>(
                                    <View style={{display:"flex",flexDirection:"row-reverse",alignItems:"center"}}>
                        ...
                        <View style={{display:"flex",flexDirection:"row",alignItems:"center",direction:"rtl"}}>
                            <Checkbox color={colorSecondary} status={eulo ? "checked" : "unchecked"} onPress={()=>setEulo(!eulo)} />
                            <Text style={{color:"red"}}>من قوانین و ضوابط را خوانده ام و ان را میپذیرم</Text>
                        </View>
                        <View style={{display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"row",direction:"rtl"}}><Text>مجموع</Text><Text style={{borderWidth:1,margin:10,paddingHorizontal:25}}>{cost}</Text></View>
                        <Button theme={{roundness:2}}  style={{width:"30%",alignSelf:"center"}}  buttonColor="#407120ff" textColor="white"> پرداخت</Button>
                    </ScrollView>
                </View>
            </View>
        </SafeAreaView>
